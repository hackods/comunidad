...
_nivel_clean = {
    "Nunca lo usé": "Nunca lo usé", "Nunca lo use": "Nunca lo usé",
...
for gen in gen_order:
    sorted_genders.extend([gen] * gen_counts.get(gen, 0))

fig = go.Figure()
seen = set()
for i, gen in enumerate(sorted_genders):
    col = i % COLS
    row = ROWS - 1 - i // COLS
    show = gen not in seen
    seen.add(gen)
    ...
    _ = fig.add_trace(go.Scatter(
        x=[col], y=[row],
        mode="markers",
        marker=dict(symbol="square", size=28, color=gen_colors_map[gen], line=dict(width=0.5, color=WHITE)),
        name=f"{gen} — {count} ({pct:.0f}%)",
        legendgroup=gen,
        showlegend=show,
        hovertemplate=f"{gen}: {count} ({pct:.0f}%)<extra></extra>",
    ))

_ = hackods_fig(fig, "")
fig.update_layout(
    xaxis=dict(visible=False, range=[-0.6, COLS - 0.4]),
    yaxis=dict(visible=False, range=[-0.6, ROWS - 0.4]),
    legend=dict(orientation="h", x=0.49, xanchor="left", y=0.6, yanchor="middle", entrywidth=150),
    plot_bgcolor=WHITE,
    height=350,
)
```
...
_ = fig.add_annotation(text=f"Promedio: {mean_age:.0f} años", x=mean_age + 1.5, y=0.95,
...
#| content: valuebox
#| title: "Edad promedio"
dict(icon="calendar-heart", color="#0a97d9", value=f"{df['edad'].mean():.0f} años")
```
...
### Género por área de conocimiento
...
fig = go.Figure(go.Choropleth(
    ...
    colorscale=[[0, WHITE]] + [[(i+1)/len(HACKODS_SEQ), c] for i, c in enumerate(HACKODS_SEQ)],
    zmin=0,
...
hackods_fig(fig, "¿De dónde venimos?")
```
...
#| content: valuebox
#| title: "Saben Python"
n_py = (df["python_nivel"] != "Nunca lo usé").sum()
...
#| content: valuebox
#| title: "Saben Quarto"
n_q = (df["quarto_nivel"] != "Nunca lo usé").sum()
...
n_gh = (df["github_nivel"] != "Nunca lo usé").sum()
...
nivel_order = ["Nunca lo usé", "Principiante", "Intermedio", "Avanzado"]
...
pct_recicla = sum(1 for p in habitos_por_persona if any("recicl" in h.lower() for h in p)) / N * 100
dict(icon="recycle", color="#4CA146", value=f"{pct_recicla:.0f}%")
```

```{python}
#| content: valuebox
#| title: "Hábitos sostenibles (prom.)"
...
_ = fig.add_annotation(text="Duermen como bebés", x=7.5, y=0.85, yref="paper",
                   showarrow=False, font=dict(size=11, color=MEDIUM_GRAY))
...
```{python}
sp_counts = df["superpoder"].value_counts().reset_index()
sp_counts.columns = ["superpoder", "count"]

fig = go.Figure(go.Treemap(
    labels=sp_counts["superpoder"],
    parents=[""] * len(sp_counts),
    values=sp_counts["count"],
    marker_colors=HACKODS_CAT[:len(sp_counts)],
    textinfo="label+value",
    textfont=dict(size=14, color=WHITE),
    textposition="middle center",
    hovertemplate="<b>%{label}</b><br>%{value} votos<extra></extra>",
